...
gulp.task("loadConfig", function () {
	platform = "web"
	if (process.argv.length >= 8 && process.argv[6] == "--config") {
		platform = process.argv[7].replace(".json", "");
	}
	let _path = `${workSpaceDir}/.laya/${platform}.json`;
	releaseDir = `${workSpaceDir}/release/${platform}`;
